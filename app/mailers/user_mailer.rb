class UserMailer < ActionMailer::Base
  ...
  def received_evaluator_application(user)
    @user = user
    mail to: user.email, subject: "10 and 5 hospitality email confirmation.", :template_path => '/user_mailer/evaluator_application'
  end
  def evaluator_application_accepted(user)
    @user = user
    mail to: user.email, subject: "Welcome to the 10 and 5 team!", :template_path => '/user_mailer/evaluator_application'
  end
  def evaluator_application_denied(user)
    @user = user
    mail to: user.email, subject: "10 and 5 hospitality application.", :template_path => '/user_mailer/evaluator_application'
  end
  ...
  def received_evaluation_application(user, evaluation)
    ...
    mail to: user.email,subject: "10 and 5 hospitality email confirmation.", :template_path => '/user_mailer/evaluation_application'
  end
  def evaluation_application_accepted(user, evaluation)
    @user = user
    @evaluation = evaluation
    mail to: user.email, subject: "10 and 5 hospitality application.", :template_path => '/user_mailer/evaluation_application'
  end
  ...
  def account_activate(user)
    @user = user
    mail to: user.email, subject: "Evaluation submitted.", :template_path => '/user_mailer/account_activation'
  end
